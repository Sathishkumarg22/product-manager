...
void main() {
  ...
  group('MyApp Widget Tests', () {
    testWidgets(
      'displays LoginScreen when AuthBloc state is AuthUnauthenticated',
    ...
    testWidgets(
      'displays ProductListScreen when AuthBloc state is AuthAuthenticated',
      (WidgetTester tester) async {
        when(
          mockAuthBloc.state,
        ).thenReturn(AuthAuthenticated(token: 'valid_token'));
        when(mockAuthBloc.stream).thenAnswer(
          (_) => Stream.value(AuthAuthenticated(token: 'valid_token')),
        );
    ...
    testWidgets('sets up MultiProvider with AuthBloc and ProductBloc', (
      WidgetTester tester,
    ) async {
      when(mockAuthBloc.state).thenReturn(AuthUnauthenticated());
      when(
    ...
    testWidgets(
      'triggers AppStarted event on AuthBloc and LoadProducts on ProductBloc',
      (WidgetTester tester) async {
        when(mockAuthBloc.state).thenReturn(AuthUnauthenticated());
        when(
          mockAuthBloc.stream,
